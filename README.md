...
Modelo de uma despesa que se repete mensalmente (ex: aluguel, assinaturas). O batch `CriarDespesasRecorrentesBatchable` usa esses registros para gerar as despesas do mês.
...
## Arquitetura Apex (SOLID)

O código Apex é organizado em camadas de responsabilidade única, cada uma com um sufixo que identifica seu papel — classes que implementam um padrão assíncrono/de entrada do Salesforce levam o nome do padrão no final (`Batchable`, `Schedulable`, `Invocable`); as demais levam o nome da própria camada arquitetural (`Repository`, `Filter`, `Service`, `Client`):

- **Filter** (`DespesaFilter`): objeto de critério de busca (fluent) para Despesa__c — monta a cláusula `WHERE` a partir de condições combináveis (status, datas, tipos de pagamento, recorrências, extração), sem saber nada de SOQL/DML em si.
- **Repository** (`DespesaRepository`, `RecorrenciaRepository`, `AsyncApexJobRepository`): único lugar que monta SOQL/DML de cada objeto. Recebe um `Filter` quando a busca precisa de critérios; nunca contém regra de negócio.
- **Service** (`CriarDespesasRecorrentesService`, `QuitarDespesasDebitoCartaoService`, `ExtracaoDespesasBigQueryService`, `BatchObservabilidadeService`): regra de negócio — decide o quê fazer, orquestrando Repository(s)/Client. Testável isoladamente, sem depender do Batchable/Invocable que o chama.
- **Client** (`BigQueryExtracaoClient`): encapsula um callout HTTP externo específico (Cloud Function do BigQuery), devolvendo a resposta crua para o Service decidir o que fazer com ela.
- **Builder** (`DespesaExtracaoPayloadBuilder`): mapeia `Despesa__c` para o formato de payload de um destino externo — puro, sem SOQL/DML/callout.
- **Batchable/Schedulable/Invocable**: classes finas que só traduzem o padrão do Salesforce (chunking de batch, agendamento cron, entrada de Flow) para uma chamada ao Service correspondente.

### CriarDespesasRecorrentesBatchable / CriarDespesasRecorrentesSchedulable

Gera as despesas do mês corrente a partir das recorrências ativas.

- `CriarDespesasRecorrentesBatchable` (`Database.Batchable` + `Database.AllowsCallouts`): `start()` busca `Recorrencia__c` ativas via `RecorrenciaRepository`; `execute()` delega ao `CriarDespesasRecorrentesService`; `finish()` loga o resultado via `BatchObservabilidadeService`.
- `CriarDespesasRecorrentesService`: calcula a data de vencimento do mês (ajustando `Dia_Vencimento__c` para o último dia do mês quando necessário), verifica idempotência via `DespesaRepository`/`DespesaFilter` (não cria uma nova despesa se já existir uma para aquela recorrência no mês corrente) e copia `Descricao__c`, `Valor__c`, `Carteira_Padrao__c`, `Tipo__c`, `Tipo_Pagamento__c`, `Variavel__c` e `Empresa__c` da recorrência para a despesa criada.
- `CriarDespesasRecorrentesSchedulable` (`Schedulable`): só dispara `Database.executeBatch(new CriarDespesasRecorrentesBatchable())`.
- Testes em `CriarDespesasRecorrentesBatchableTest` cobrem: criação para recorrência ativa, não duplicação ao rodar o batch duas vezes no mesmo mês, não criação para recorrência inativa, e o `Schedulable` disparando o batch.
- `scripts/apex/schedule-batch.apex`: setup único, agenda via `System.schedule` (dia 1 de cada mês, 01:00).
...
### QuitarDespesasDebitoCartaoBatchable / QuitarDespesasDebitoCartaoSchedulable

Quita automaticamente as despesas de `Débito Automático`/`Cartão de Crédito` no dia do vencimento.

- `QuitarDespesasDebitoCartaoBatchable` (`Database.Batchable` + `Database.AllowsCallouts`): `start()` monta um `DespesaFilter` (`Status__c = 'Pendente'`, `Data_Vencimento__c = hoje`, `Tipo_Pagamento__c` em `Débito Automático`/`Cartão de Crédito`) e busca via `DespesaRepository`; `execute()` delega ao `QuitarDespesasDebitoCartaoService`; `finish()` loga o resultado via `BatchObservabilidadeService`.
- `QuitarDespesasDebitoCartaoService`: atualiza `Status__c = 'Pago'` e `Data_Pagamento__c = hoje` nas despesas recebidas. Não mexe em despesas já `Pago`/`Cancelado` nem em outros tipos de pagamento — isso já é garantido pelo filtro usado no `start()`.
- `QuitarDespesasDebitoCartaoSchedulable` (`Schedulable`): só dispara `Database.executeBatch(new QuitarDespesasDebitoCartaoBatchable())`.
...
- Testes em `QuitarDespesasDebitoCartaoBatchableTest` cobrem: quitação de Débito Automático e Cartão de Crédito vencendo hoje, não quitação de outros tipos de pagamento, não quitação de vencimento futuro, não alteração de despesa já paga, e o `Schedulable` disparando o batch.
- `scripts/apex/schedule-quitar-debito-cartao.apex`: setup único, agenda via `System.schedule` (todo dia às 6h, horário padrão da org).
...
Stack: **Grafana Cloud (Loki)**, plano gratuito. Escopo: as duas batches acima, as ações de escrita do MCP (`criar_despesa`, `quitar_despesa`) e a extração sob demanda para o BigQuery (`ExtracaoDespesasBigQueryService`, ver [Extração para BigQuery (MVP4)](#extração-para-bigquery-mvp4)). Plano completo de decisão em `claudeplanning/plano-observabilidade.md` (pasta local, gitignorada).
...
- **Lado Apex** (`LokiLoggerService.cls`): callout HTTP feito no `finish()` de cada batch (não em `start()`, por causa das restrições de callout+DML na mesma transação), usando o Named Credential `Grafana_Loki` (`force-app/main/default/namedCredentials/Grafana_Loki.namedCredential-meta.xml`, protocolo Password Authentication → gera o header `Authorization: Basic` automaticamente). Nunca lança exceção — uma falha ao logar não pode derrubar a batch. A consulta a `AsyncApexJob` (status/contagem de itens/erros) fica no `AsyncApexJobRepository`; quem liga os dois é o `BatchObservabilidadeService.logResultado(origem, jobId)`, chamado pelo `finish()` de cada Batchable — evita duplicar essa lógica nos dois batches.
- **Testado com mock** (`LokiLoggerServiceMock.cls`, `@isTest`): `LokiLoggerServiceTest` cobre resposta de sucesso, resposta de erro HTTP e callout sem mock — em todos os casos `LokiLoggerService.log()` não propaga exceção. Os testes dos dois batches (`QuitarDespesasDebitoCartaoBatchableTest`, `CriarDespesasRecorrentesBatchableTest`) usam o mesmo mock.
...
- **`ExtracaoDespesasBigQueryInvocable.cls`**: só traduz Input/Output do Flow (`ExtracaoInput`/`ExtracaoResultado`, via `@InvocableMethod`) para o `ExtracaoDespesasBigQueryService` — nenhuma regra de negócio na própria classe.
- **`ExtracaoDespesasBigQueryService.cls`**: orquestra a extração — busca via `DespesaRepository`/`DespesaFilter` (`Data_Extracao__c = null` + `Data_Vencimento__c` no intervalo do mês escolhido, com `Carteira__r.Name`/`Tipo__c`/`Instituicao__c`), monta o payload via `DespesaExtracaoPayloadBuilder`, envia via `BigQueryExtracaoClient` e só em caso de sucesso faz `UPDATE Data_Extracao__c = Date.today()` nos registros processados (via `DespesaRepository`).
- Testado em `ExtracaoDespesasBigQueryInvocableTest.cls` com `HttpCalloutMock` (reaproveita `LokiLoggerServiceMock`), cobrindo mês certo/errado, idempotência e falha de callout — mais `DespesaExtracaoPayloadBuilderTest` isolando a montagem do payload (ISO 8601, campos de carteira ausentes) sem SOQL/DML.
...
- **Log de observabilidade**: `ExtracaoDespesasBigQueryService.extrairMes()` envia um evento ao Grafana Loki (`LokiLoggerService`, origem `ExtracaoDespesasBigQueryService`, tipo de evento `extracao_despesas`) ao final de cada rodada — status `sucesso`/`falha`/`sem_dados`, mês de referência (ISO) como identificador de rastreio, e `detalhes` com `quantidadeExtraida` e (quando houve callout ao GCP) o `statusHttp` retornado. O log de sucesso é enviado **antes** do `UPDATE Data_Extracao__c` (Apex não permite callout depois de DML na mesma transação). Testado em `ExtracaoDespesasBigQueryInvocableTest` (`testExtracaoComSucessoEnviaLogComQuantidadeExtraida`, `testExtracaoSemDespesasPendentesEnviaLogComoSemDados`, `testExtracaoComFalhaNoCalloutEnviaLogComoFalha`), capturando o corpo da requisição feita ao endpoint do Loki.
...
**No ar e validado ponta a ponta com dado real**: campo `Data_Extracao__c`, Named Credential, `ExtracaoDespesasBigQueryInvocable`/`ExtracaoDespesasBigQueryService` (+Tests), Flow, FlexiPage/tab `Admin_Page`, Cloud Function, dataset/tabela/view no BigQuery. A primeira extração real (agosto/2026) processou 124 despesas com sucesso, confirmadas no BigQuery com datas em ISO corretas; uma segunda chamada no mesmo mês extraiu 0 (idempotência confirmada). MVP5 (dashboards Looker Studio) ainda não iniciado — só a view `despesas_atual` está pronta para conectar.
...
**Confirmação obrigatória antes de gravar**: `criar_despesa`, `quitar_despesa` e `editar_despesa` só persistem no Salesforce quando chamadas com `confirmar = true`. Sem isso, a ferramenta faz todas as validações normalmente mas devolve uma prévia (`confirmacaoNecessaria: true` + os campos que seriam gravados, ou atual x novo no caso de `editar_despesa`) sem tocar no Salesforce. O Claude deve sempre mostrar essa prévia ao usuário e só rechamar a ferramenta com `confirmar = true` depois de confirmação explícita — pensado para reduzir erros de categorização (ex: `Tipo__c` errado) em despesas avulsas.
...
- `criar_despesa` — cria uma `Despesa__c` avulsa (sem `Recorrencia__c`) a partir de um comprovante de pagamento já efetuado (foto de nota/recibo ou comprovante de transação por aproximação/NFC no celular), já com `Status__c = 'Pago'` e `Data_Pagamento__c` preenchidos. Antes de criar, o Claude deve chamar `identificar_despesa_por_comprovante` para descartar que o comprovante seja de uma despesa pendente já existente. `carteiraId` nunca é adivinhado: para comprovante de Débito/Pix, o Claude busca carteiras com `Tipo__c = 'Conta Corrente'` e pergunta ao usuário qual usar; para Cartão de Crédito, busca `Tipo__c = 'Cartão de Crédito'` e pergunta qual cartão cadastrado usar — mesmo havendo só uma opção. Só grava com `confirmar = true`; sem isso, devolve uma prévia dos campos a persistir.
- `editar_despesa` — corrige um ou mais campos (`descricao`, `valor`, `dataVencimento`, `dataPagamento`, `status`, `carteiraId`, `tipo`, `tipoPagamento`, `empresa`, `observacoes`, `variavel`) de uma `Despesa__c` já existente, para quando o Claude classificar algo errado ao criar uma despesa avulsa (ex: `Tipo__c`). Só grava com `confirmar = true`; sem isso, devolve uma prévia com valor atual x valor novo de cada campo informado.
...
sf apex run test --target-org <alias> --class-names CriarDespesasRecorrentesBatchableTest,QuitarDespesasDebitoCartaoBatchableTest,ExtracaoDespesasBigQueryInvocableTest,LokiLoggerServiceTest,DespesaFilterTest,DespesaExtracaoPayloadBuilderTest --synchronous
```

## Histórico de mudanças

- **2026-09-06** — Adicionado log de observabilidade à extração para o BigQuery: `ExtracaoDespesasBigQueryService.extrairMes()` agora envia um evento ao Grafana Loki (`LokiLoggerService`) ao final de cada rodada, com status (`sucesso`/`falha`/`sem_dados`), mês de referência e quantidade de despesas extraídas. Ver seção [Extração para BigQuery (MVP4)](#extração-para-bigquery-mvp4). Deploy e 32/32 testes passando na org `financeiro-dev`.
- **2026-09-06** — Refatorado todo o código Apex seguindo SOLID (responsabilidade única por classe), com camadas nomeadas por padrão: `Filter` (`DespesaFilter`, critério de busca fluente), `Repository` (`DespesaRepository`, `RecorrenciaRepository`, `AsyncApexJobRepository`, únicos lugares com SOQL/DML), `Service` (`CriarDespesasRecorrentesService`, `QuitarDespesasDebitoCartaoService`, `ExtracaoDespesasBigQueryService`, `BatchObservabilidadeService`, com a regra de negócio), `Client` (`BigQueryExtracaoClient`, callout isolado) e `Builder` (`DespesaExtracaoPayloadBuilder`, mapeamento puro). Classes que implementam um padrão assíncrono/de entrada do Salesforce foram renomeadas para carregar o nome do padrão no final: `CriarDespesasRecorrentesBatch` → `CriarDespesasRecorrentesBatchable` + `CriarDespesasRecorrentesSchedulable` (batch e agendamento separados em classes distintas — antes eram a mesma classe implementando as duas interfaces); mesma separação em `QuitarDespesasDebitoCartaoBatch` → `QuitarDespesasDebitoCartaoBatchable` + `QuitarDespesasDebitoCartaoSchedulable`; `ExtracaoDespesasBigQuery` → `ExtracaoDespesasBigQueryInvocable` (fina, só tradução Flow↔Service) + `ExtracaoDespesasBigQueryService`; `LokiLogger` → `LokiLoggerService` (mesmo comportamento, só renomeado por consistência). Toda a cobertura de teste original foi preservada nas classes renomeadas (`CriarDespesasRecorrentesBatchableTest`, `QuitarDespesasDebitoCartaoBatchableTest`, `ExtracaoDespesasBigQueryInvocableTest`, `LokiLoggerServiceTest`) e novos testes puros (sem SOQL/DML) foram adicionados para as camadas isoláveis (`DespesaFilterTest`, `DespesaExtracaoPayloadBuilderTest`), incluindo um teste do `Schedulable` de cada batch. `scripts/apex/*.apex` e o Flow `Extracao_Despesas_Mensal` (`actionName`) atualizados para as novas classes. Os dois jobs agendados na org (`Criar Despesas Recorrentes - Mensal`, `Quitar Despesas Debito Automatico e Cartao - Diario`) foram abortados e reagendados apontando para os novos `Schedulable`s antes de remover as classes antigas. Deploy e testes rodados na org `financeiro-dev`.
- **2026-09-06** — Adicionada a ferramenta `editar_despesa` ao `mcp-server`, para o Claude corrigir campos de uma `Despesa__c` já existente quando algo foi classificado errado ao criar uma despesa avulsa (ex: `Tipo__c`). E adicionado o parâmetro `confirmar` a `criar_despesa`, `quitar_despesa` e `editar_despesa`: sem `confirmar = true` nenhuma das três grava no Salesforce — devolvem uma prévia dos campos que seriam persistidos (atual x novo, no caso de `editar_despesa`) para o usuário conferir antes de o Claude chamar de novo com `confirmar = true`. Objetivo: reduzir erros de qualidade de dado nas despesas criadas via MCP. Ver seção [Integração MCP customizada](#integração-mcp-customizada-conciliação-de-comprovantes).
